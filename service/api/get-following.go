...
func (rt *_router) getMyFollowing(w http.ResponseWriter, r *http.Request, ps httprouter.Params, ctx reqcontext.RequestContext) {
	// get user profile following from the url
	id_user := ps.ByName("id")

	// get user profile followers from the database
	followers, err := rt.db.GetFollowing(id_user)
	if err != nil {
