...
func (rt *_router) getProfile(w http.ResponseWriter, r *http.Request, ps httprouter.Params, ctx reqcontext.RequestContext) {
	...
	profile.Followers, err = rt.db.GetFollow(id_user)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		ctx.Logger.Errorf("error getting followers")
		return
	}
	
	profile.Following, _ = rt.db.GetFollowing(id_user)
	profile.Photos, _ = rt.db.GetListPhoto(id_user)
	profile.Nickname, _ = rt.db.GetNickname(id_user)
