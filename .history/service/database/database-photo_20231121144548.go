...
func (db *appdbimpl) GetPhoto(id_photo int64) (Photo, error) {
	var photo Photo
	err := db.c.QueryRow(`SELECT * FROM photo WHERE id_photo = ?`, id_photo).Scan(&photo.Id_photo, &photo.User_ID, &photo.Date)
