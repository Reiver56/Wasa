...
func (db *appdbimpl) GetFollow(id_user string) ([]User, error) {
	rows, err := db.c.Query("SELECT id, nickname FROM User WHERE id IN (SELECT follower FROM follow WHERE followed = ?)", id_user)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *appdbimpl) GetFollowing(id_user string) ([]User, error) {
	rows, err := db.c.Query("SELECT id, nickname FROM User WHERE id IN (SELECT followed FROM follow WHERE follower = ?)", id_user)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
