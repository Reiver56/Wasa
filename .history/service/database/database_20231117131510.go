...
func New(db *sql.DB) (AppDatabase, error) {
	...
	err := db.QueryRow(`SELECT count(name) FROM sqlite_master WHERE type='table' AND name='example_table';`).Scan(&tableName)
	if errors.Is(err, sql.ErrNoRows) {
		sqlStmt := `CREATE TABLE User (id TEXT NOT NULL PRIMARY KEY);`
