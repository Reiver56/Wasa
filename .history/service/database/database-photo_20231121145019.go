...
func (db *appdbimpl) GetPhoto(tphoto_id int64, id_req string) (Photo, error) {
	var photo Photo
	err := db.c.QueryRow(`SELECT * FROM photo WHERE id_photo = ? AND user_id NOT IN (SELECT banner FROM ban WHERE banned = ? ) `, tphoto_id ).Scan(&photo.Id_photo, &photo.User_ID, &photo.Date)
