...
func (db *appdbimpl)DeleteComment(id_photo int64, id_user string, text string) (int64, error) {
	
	res, err := db.c.Exec(`DELETE FROM comments WHERE (id_comment INTEGER PRIMARY KEY AUTOINCREMENT,
		id_photo INTEGER NOT NULL,
		id_user VARCHAR(16) NOT NULL,)`, id_photo, id_user, text)
	if err != nil {
		return -1,err
	}
	comment_id, err := res.LastInsertId()
