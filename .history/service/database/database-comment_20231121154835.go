...
func (db *appdbimpl)DeleteComment(id_photo int64, id_user string, id_comment int64) (int64, error) {
	
	res, err := db.c.Exec(`DELETE FROM comments WHERE (id_comment = ?, id_photo = ?, id_user = ?)`, id_photo, id_user, id_comment)
	if err != nil {
		return -1,err
	}
	comment_id, err := res.LastInsertId()
