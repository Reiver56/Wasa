...
func (db *appdbimpl) GetMyStream(user User) ([]Ph) {
	rows, err := db.c.QueryRow("SELECT * FROM photo WHERE user_id IN (SELECT followed FROM follow WHERE follower = ?) ORDER BY date DESC", user.ID)
	if err != nil {
		// Count always returns a row thanks to COUNT(*), so this situation should not happen
		return true
	}
