...
func (db *appdbimpl) GetPhoto(id string, id_req string) (Photo, error) {
	var photo Photo
	err := db.c.QueryRow(`SELECT * FROM photo WHERE user_id = ?`, id_photo).Scan(&photo.Id_photo, &photo.User_ID, &photo.Date)
