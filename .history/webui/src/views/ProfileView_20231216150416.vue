<template>
	...
	<div class = "top-container-profile">
		...
		<div v-if="showPosts" class="post-view" @click.self="exitPost">
			<PostView :data="postViewData" />
		</div>

		<div class = "row">
				<div class="col-12 d-flex justify-content-center"><input id="fileUploader" type="file" class="profile-file-upload" @change="uploadPhoto" accept=".jpg, .png">
			<label v-if=""  class="btn my-btn-add-photo ms-2 d-flex align-items-center" for="fileUploader"> Add </label>
		</div></div>



		<div class="posts-container">
			<span v-if="(posts.length == 0 && dataAvailable) && !showPosts" class="no-posts">No posts yet</span>
			<div v-if="posts.length > 0" class="posts-container-grid">
				<div v-for="post in posts" class="posts-container-grid-item" @click="openPhoto(post)">
					<img :src="post.url" class="posts-container-grid-item-img">
				</div>
		</div>
	</div>
</div>
...
export default {
	...
	methods: {
		async uploadPhoto(){
			...
			let reader = new FormReader();

			reader.readAsArrayBuffer(photo)
			reader.onload = async () => {
				let response = await this.$axios.post(`users/${localStorage.userID}/photos`, reader.result, { headers: { 'Authorization': `${localStorage.token}` } });
