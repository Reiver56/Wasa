	...
	<div class = "top-container-profile">
		...
		<div class = "posts-container">
			...
			<div v-for="post in posts" :key="post.id">
				<img :src="`data:image/jpg;base64${photo.image}`" loading="lazy" class="post-image" @click="openPhoto(post) " :id="post.id"/>
			</div>
		</div>
</div>
...
export default {
	...
	methods: {
		async uploadPhoto(){
			...
			formData.append("image", postData['fileUploader']);
		...
		async getPhotos(){
			this.isLoading = true;
			try{
				let response = await this.$axios.get(`users/${this.$route.params.id}/photos`, { headers : { 'Authorization': `${localStorage.token}` } });
				this.posts = response.data;
				this.photosCount = response.data.length;
