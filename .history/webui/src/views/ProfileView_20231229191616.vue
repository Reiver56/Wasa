	...
	methods: {
		...
		getProfile: async function() {
			...
			try{
				...
				this.isFollowed = response.data.followers != null ? response.data.followers.find(obj =>localStorage.UserID) : false;
				this.followButton = this.isFollowed == true ? "Follow" : "Unfollow";
			}
			catch(e){
		...
		async follow(){
			...
			try {
				if(this.isFollowed){
					await this.$axios.delete(`users/${this.userID}/followers/${localStorage.UserID}`followStatusfollowStatusfollowStatus);
					this.followersCount--;
				}else{
