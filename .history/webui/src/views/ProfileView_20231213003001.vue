	...
	methods: {
		getProfile: async function() {
			...
			try{
				...
				this.followersCount = response.data.followers != null ? response.data.followers : 0;
				this.followingCount = response.data.following != null ? response.data.followers : 0;
		...
		async follow(){
			if (this.isFollowed){
				...
				try{
					...
					let response = await this.$axios.put(`users/${localStorage.UserID}/followers/${this.userID}`);
