...
export default {
	...
	methods: {
		getProfile: async function() {
			this.isLoading = true;
			try{
				console.log(this.$route.User_id);
				let response = await this.$axios.get(`users/${this.User_id}`, { headers : { 'Authorization': `${localStorage.token}` } });
		...
		async getPhotos(){
			this.isLoading = true;
			try{
				let response = await this.$axios.get(`users/${this.User_id}/photos`, { headers : { 'Authorization': `${localStorage.token}` } });
