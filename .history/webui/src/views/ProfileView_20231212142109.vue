<template>
	...
	<div class = "top-container-profile">
		<div class="top-body-profile-container">
			<button class="option-button" @click="option = true, console.log(isOwner)" @focusout="closeOption">
				<font-awesome-icon icon="fa-solid fa-ellipsis-v" />
			</button>
			<div v-if="options && isOwner" class="option-container">
				<button class="option-button" @click="getBans">Ban User</button> </div>
...
export default {
	...
	methods: {
		...
		getBans() {
			...
			this.dataGet = async (usersArray, dataAvailable) => {
				try{
					let response = await this.$axios.get(`users/${this.userID}/bans`, { headers : { 'Authorization': `${localStorage.token}` } });
					usersArray = response.data;
					dataAvailable = true;
					usersArray.push(...response.data);
				}
