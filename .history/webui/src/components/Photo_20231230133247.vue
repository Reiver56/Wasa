...
<template>
	<div class="photo-container">
		<LikeModal :model_id="'like_modal'+photo_id"
			:likes="Likes" />
		<CommentModal :model_id="'comment_modal'+photo_id"
			:comments="Comments"
			:photo_owner="owner"
			:photo_id="photo_id"
					...
					<div class="container">
						<div class="d-flex flex-row justify-content-end align-items-center mb-2">
							...
							<button class="my-trnsp-btn-comment" @click="commentClick" data-bs-toggle="modal" :data-bs-target="'#comment_modal'+photo_id">
								...
								<div>{{Comments != null ? Comments.length : 0}}</div>{{Comments != null ? Comments.length : 0}}
                            </button>
						</div>
