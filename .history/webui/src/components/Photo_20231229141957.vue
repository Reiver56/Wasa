...
export default{
	...
	methods: {
		...
		async toggleLike(){
			...
			try{
				if (this.liked){
					await this.$axios.delete(`users/${this.owner}/photos/${this.photo_id}/likes`, {
						headers: {
							Authorization: `${localStorage.token}`
						}
					});
					...
					await this.$axios.put(`users/${this.owner}/photos/${this.photo_id}/likes`, this.Likes.push({
						user_id: localStorage.token,}), {headers: {
							Authorization: `${localStorage.token}`
						}
					});
					this.liked = true;
