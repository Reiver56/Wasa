<template>
	 <div class="upload-form-background" @click.self="this.$emit('exit-form')">
        ...
        <EditorPost :image64="file64" :editorType="this.$props.photoType" v-if="file64"
            @exit-form="this.$emit('exit-form')" @save-upload-form="saveData" />
    </div>
</template>
...
export default{
	...
	methods: {
		...
		async createPost(postData) {
			...
			try{
				let response = await this.$axios.post(`/users/${localStorage.userID}/photos`, formData,
